...
_Logger.deprecated = function(original, replacement) {
  _Logger.deprecatedWithMsg(original, "Please use '" + replacement + "' instead.");
};
_Logger.deprecatedWithMsg = (funcName, msg) => {
  if (_Logger.shouldLog(1 /* Error */)) {
    _Logger.logErrorHandler("Ably: Deprecation warning - '" + funcName + "' is deprecated and will be removed from a future version. " + msg);
  }
};
...
function createMissingModuleError(moduleName) {
...
var EventEmitter = class _EventEmitter {
  ...
  whenState(targetState, currentState, listener, ...listenerArgs) {
    ...
    if (typeof listener !== "function") {
      return new Promise((resolve) => {
        _EventEmitter.prototype.whenState.apply(this, [targetState, currentState, resolve].concat(listenerArgs));
      });
    }
...
function betterTransportThan(a, b) {
  return Platform.Defaults.transportPreferenceOrder.indexOf(a.shortName) > Platform.Defaults.transportPreferenceOrder.indexOf(b.shortName);
}
...
var ConnectionManager = class _ConnectionManager extends eventemitter_default {
  ...
  tryATransport(transportParams, candidate, callback) {
    transport_default.tryConnect(this.supportedTransports[candidate], this, this.realtime.auth, transportParams, (wrappedErr, transport) => {
  ...
  setTransportPending(transport, transportParams) {
    ...
    transport.once("connected", (error, connectionId, connectionDetails) => {
      if (mode == "upgrade" && this.activeProtocol) {
        if (transport.shortName !== optimalTransport && this.getUpgradePossibilities().includes(optimalTransport) && this.activeProtocol) {
          setTimeout(() => {
  ...
  activateTransport(error, transport, connectionId, connectionDetails) {
    ...
    this.proposedTransports.slice().forEach((proposedTransport) => {
      if (proposedTransport === transport) {
        logger_default.logAction(logger_default.LOG_ERROR, "ConnectionManager.activateTransport()", "Assumption violated: activating a transport that is still marked as a proposed transport; transport = " + transport.shortName + "; stack = " + new Error().stack);
        arrDeleteValue(this.proposedTransports, transport);
      } else {
  ...
  upgradeIfNeeded(transportParams) {
    ...
    upgradePossibilities.forEach((upgradeTransport) => {
  ...
  async processChannelMessage(message, transport) {
    const onActiveTransport = this.activeProtocol && transport === this.activeProtocol.getTransport(), onUpgradeTransport = this.pendingTransports.includes(transport) && this.state == this.states.synchronizing;
    if (onActiveTransport || onUpgradeTransport) {
      await this.realtime.channels.processChannelMessage(message);
    } else {
      if ([actions2.ACK, actions2.NACK, actions2.ERROR].includes(message.action)) {
        await this.realtime.channels.processChannelMessage(message);
      } else {
      }
    }
  ...
  persistTransportPreference(transport) {
    var _a2, _b;
    if (defaults_default.upgradeTransports.includes(transport.shortName)) {
...
var Connection = class extends eventemitter_default {
  constructor(ably, options) {
    ...
    this.whenState = (state, listener) => {
      return eventemitter_default.prototype.whenState.call(this, state, this.state, listener, new connectionstatechange_default(void 0, state));
    };
    this.ably = ably;
  ...
  get recoveryKey() {
    logger_default.deprecated("Connection.recoveryKey attribute", "Connection.createRecoveryKey() method");
...
function createMissingImplementationError() {
  return new ErrorInfo("No HTTP request module provided. Provide at least one of the FetchRequest or XHRRequest modules.", 400, 4e4);
}
var _a;
var Http2 = (_a = class {
  constructor(client) {
